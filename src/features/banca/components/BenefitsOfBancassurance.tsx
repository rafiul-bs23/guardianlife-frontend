...
export function BenefitsOfBancassurance() {
  return (
    <div className="flex flex-col lg:flex-row items-center lg:items-stretch justify-center gap-8 lg:gap-[24px] px-4">
      {bancassurance_benefits_sections?.map((section: BenefitSection) => (
        <div
          key={section?.id}
          className={`w-full max-w-[730px] h-auto lg:h-[856px] ${section?.bg_color} rounded-[35px] lg:px-[42px] px-6 py-[48px] flex flex-col`}
        >
          ...
          <div className="flex flex-col gap-[12px] mt-[23px]">
            {section?.points?.map((point: string, index: number) => (
              <div key={index} className="flex items-start gap-[10px]">
                <span
                  className={`mt-2 w-[20px] h-[20px] rounded-full ${section?.dot_color} flex-shrink-0 mt-[5px]`}
                />
